...
if __name__ == "__main__":
    ...
    data, id2name = load_aisec_single_gml("aes_key_expand_128_modified_wfeatures.gml")
    ...
    train_loader = GraphSAINTRandomWalkSampler(data, batch_size=3000, walk_length=3, shuffle=True, sample_coverage=50)
    morun_training(data, train_loader, in_dim, out_dim, id2name)    

    # Save predictions to a new GML for Gephi analysis
    save_predictions_to_gml(
        original_gml_path="aes_key_expand_128_modified_wfeatures.gml",
        data=data,
        model=model,
        id2name=id2name,
        output_gml_path="gml_results.gml"
    )