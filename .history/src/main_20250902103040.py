...
if __name__ == "__main__":
    ...
    train_loader = GraphSAINTRandomWalkSampler(data, batch_size=500, walk_length=3, shuffle=True)
    # run_training(data, train_loader, in_dim, out_dim)    
    ...
    model.eval()
    with torch.no_grad():
        embeddings = model(data.x, data.edge_index).cpu().numpy()  # shape: [num_nodes, out_dim]

    tsne = TSNE(n_components=2, random_state=42)
    z_tsne = tsne.fit_transform(embeddings)

    plt.figure(figsize=(8, 6))
    plt.scatter(z_tsne[:, 0], z_tsne[:, 1], c=data.y.cpu().numpy(), cmap='tab20', s=5)
    plt.title("t-SNE of GNN Node Embeddings")
    plt.xlabel("t-SNE dim 1")
    plt.ylabel("t-SNE dim 2")
    plt.colorbar(label="Subcircuit class")
    plt.tight_layout()
