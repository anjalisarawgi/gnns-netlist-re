...
def classwise_accuracy(model, data, mask, id2subcircuit=None):
    model.eval()
    out = model(data.x, data.edge_index)
    pred = out.argmax(dim=1)
    ...
    for c in range(num_classes):
        ...
        # acc_per_class[c] = acc
        label = id2subcircuit[c] if id2subcircuit else c
...
def run_training(data, train_loader, in_dim, out_dim):
    ...
    for epoch in range(1, epochs+1):
        ...
        if epoch % 100 == 0 or epoch == 1:
            classwise_acc = classwise_accuracy(model, data, data.val_mask)
    ...
    classwise_acc = classwise_accuracy(model, data, data.test_mask)
...
if __name__ == "__main__":
    ...
    data, subcircuit2id = load_aisec_single_gml("aes_key_expand_128_modified_wfeatures.gml")
    ...
    id2subcircuit = {v: k for k, v in subcircuit2id.items()}
    train_loader = GraphSAINTRandomWalkSampler(data, batch_size=3000, walk_length=3, shuffle=True, sample_coverage=50)
    run_training(data, train_loader, in_dim, out_dim)    