...
import random
def load_aisec_single_gml(dataset_name, gml_path):
    print("calling gnn from path:", gml_path)
    random.seed(42)

    if dataset_name =="aisec_single":
        gml_path = gml_path
        G = nx.read_gml(gml_path, label="id")
        nodes = list(G.nodes())
    elif dataset_name == "aisec_all":
        continue
...
def run_tsne(model, data, dataset_name):
    ...
    if dataset_name == "aisec_single":
        ...
        label_names = [str(int(label)) for label in data.y.cpu().numpy()]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", default="aisec_single", choices = ["aisec_single", "aisec_all", "GNN_RE_single", "GNN_RE_all"])
    parser.add_argument("--gml_path", default="mwe/aes_cipher_top_gephi.gml")
    parser.add_argument("--model_name", default="gat", choices-'')
    ...
    data = load_aisec_single_gml(args.dataset_name, args.gml_path)
