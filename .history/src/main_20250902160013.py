...
@torch.no_grad()
def evaluate_multiclass(model, data, mask, label_map):
    model.eval()
    out = model(data.x, data.edge_index)
    pred = out.argmax(dim=1)
...
def run_training(data, train_loader, in_dim, out_dim, task_type):
    ...
    if task_type == "binary":
        ...
        for epoch in range(1, epochs+1):
            loss = train(model, train_loader, optimizer)
            train_metrics = evaluate_multiclass(model, data, data.train_mask,  data.label_id_map)
            val_metrics = evaluate_multiclass(model, data, data.val_mask,  data.label_id_map)
            print(f"Final Train Accuracy: {train_metrics:.4f}")
            print(f"Final Val Accuracy: {test_acc:.4f}")

        test_acc = evaluate_multiclass(model, data, data.test_mask, data.label_id_map)
...
if __name__ == "__main__":
    ...
    in_dim = data.num_features
    out_dim = 2 if args.task_type == "binary" else len(data.label_id_map)
