...
def evaluate(model, data):
    ...
    return {"f1": f1, "precision": precision, "recall": recall, "total_acc": acc, "class1_acc": acc1, "class0_acc": acc0, "y_pred": pred}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_type", default="gat", choices=["graphsage", "gcn", "gat", "graphTransformer"])

    # 
    parser.add_argument("--test_gml", nargs="+", default=None)
    parser.add_argument("--test_files", type=str, default=None, help="Path to test files (json)")
    parser.add_argument("--label_mode", default="boundary", choices=["subcircuit_name", "boundary"])
    args = parser.parse_args()


    #  folder name = model_path
    model_name = args.model_pat
    ...
    # two ways of loading test graphs either on confic or on command line
    test_graphs = []
    if args.test_files:
    ...
    for gml_path in test_graphs:
        ...
        data, id2label, num_nodes, num_edges = load_aisec_single_gml(gml_path, binary_label=True, label_mode=args.label_mode)
        in_dim = data.num_features
        out_dim = 2
        model = load_model(model_type=args.model_type, in_dim=in_dim, out_dim=out_dim, model_path=args.model_path)
        ...
        save_predictions_to_gml(original_gml_path=gml_path, id2name=id2label, preds=metrics["y_pred"], out_path=out_file)
