...
def random_netIDError(G, error_rate = 0.01): # 0.01 = 1%
...
def layout_netIDError(G, error_rate = 0.01, layout_radius = 5):
    G_error = copy.deepcopy(G)
    edges = list(G_error.edges())
    nodes = list(G_error.nodes())
...
def speckled_gateIDError(G, error_rate=0.01, min_area=2, max_area=5):
    ...
    while len(marked) < total_to_mark and attempts < max_attempts:
        ...
        cluster = list(nx.bfs_tree(G_annotated, source=seed, depth_limit=2).nodes())[:cluster_size]
